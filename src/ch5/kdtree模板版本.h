...
class KdTree {
   public:
    ...
    bool BuildTree(const TCloudPtr& cloud);

    /// 获取k最近邻
    bool GetClosestPoint(const TPointType& pt, std::vector<int>& closest_idx, int k = 5);

    /// 并行为点云寻找最近邻
    bool GetClosestPointMT(const TCloudPtr& cloud, std::vector<std::pair<size_t, size_t>>& matches, int k = 5);
