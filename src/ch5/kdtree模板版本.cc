...
template <class TCloudPtr,class TPointType>
bool KdTree<TCloudPtr,TPointType>::BuildTree(const TCloudPtr &cloud) {
    if (cloud->empty()) {
        return false;
    }
...
template <class TCloudPtr,class TPointType>
void KdTree<TCloudPtr,TPointType>::Insert(const IndexVec &points, KdTreeNode *node) {
...
template <class TCloudPtr,class TPointType>
bool KdTree<TCloudPtr,TPointType>::GetClosestPoint(const TPointType &pt, std::vector<int> &closest_idx, int k) {
    if (k > size_) {
...
template <class TCloudPtr,class TPointType>
bool KdTree<TCloudPtr,TPointType>::GetClosestPointMT(const TCloudPtr &cloud, std::vector<std::pair<size_t, size_t>> &matches, int k) {
...
template <class TCloudPtr,class TPointType>
void KdTree<TCloudPtr,TPointType>::Knn(const Vec3f &pt, KdTreeNode *node, std::priority_queue<NodeAndDistance> &knn_result) const {
    if (node->IsLeaf()) {
        // 如果是叶子，检查叶子是否能插入
...
template <class TCloudPtr,class TPointType>
bool KdTree<TCloudPtr,TPointType>::NeedExpand(const Vec3f &pt, KdTreeNode *node, std::priority_queue<NodeAndDistance> &knn_result) const {
...
template <class TCloudPtr,class TPointType>
void KdTree<TCloudPtr,TPointType>::ComputeDisForLeaf(const Vec3f &pt, KdTreeNode *node,
...
template <class TCloudPtr,class TPointType>
bool KdTree<TCloudPtr,TPointType>::FindSplitAxisAndThresh(const IndexVec &point_idx, int &axis, float &th, IndexVec &left, IndexVec &right) {
...
template <class TCloudPtr,class TPointType>
void KdTree<TCloudPtr,TPointType>::Reset() {
    tree_node_id_ = 0;
...
template <class TCloudPtr,class TPointType>
void KdTree<TCloudPtr,TPointType>::Clear() {
...
template <class TCloudPtr,class TPointType>
void KdTree<TCloudPtr,TPointType>::PrintAll() {
    for (const auto &np : nodes_) {
        auto node = np.second;
